...
{
  var helpers = [],
      helperCollection = require('./spec-collection').create();
...
jasmine.executeSpecsInFolder = function(folder,
  ...
  var fileMatcher = matcher || new RegExp(".(js)$", "i"),
      ...
      specs = require('./spec-collection').create(),
  ...
  if (useRequireJs) {
    require('./requirejs-runner').executeJsRunner(specs, done, jasmineEnv, requireJsBaseUrl);
  } else {
